...
export const api: Api = {
  ...
  setAuthPersistence: async (): Promise<void> => {
    try {
      ...
      try {
        ...
        // If even NONE fails, we log it, but we catch it so it doesn't bubble up.
        // Bubbling up might cause the caller to leave persistence as LOCAL (default), which crashes.
        console.error("Failed to set in-memory persistence:", innerError);
      }
    }
  },
  ...
  async getGradeGoals(): Promise<Record<string, GoalDetail>> {
    ...
    snapshot.forEach((snap) => {
      ...
      const originalKey = Object.values(GradeLevel).find(g => g.replace(/\//g, '-') === snap.id) || snap.id;
      goals[originalKey] = {
  ...
  async setGradeGoal(grade: GradeLevel, goal: GoalDetail): Promise<void> {
    ...
    // Replaces '/' with '-' to handle '7th/8th Grade', as '/' is invalid in doc IDs.
