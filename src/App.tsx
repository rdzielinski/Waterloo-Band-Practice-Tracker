...
import { mockUserProfile, generateMockEntries, mockGoals, generateCommunityMockEntries } from './lib/mockData';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isGuestMode) {
        setIsLoading(false);
        // setAllPracticeEntries handled in handleEnterGuestMode
        return;
    }
    const loadData = async () => {
      ...
      try {
        if (userProfile.role === 'teacher') {
          ...
          // We wrap ALL fetches in individual try/catch blocks to avoid crashing the entire app
          // if one request fails (e.g. permission error).
          
          let studentEntries: PracticeEntry[] = [];
          try {
             studentEntries = await api.getStudentEntriesByUserId(currentUser.uid);
          } catch (e) {
             console.error("Error fetching student entries:", e);
             addToast("Could not load your practice entries. Please reload or check your connection.", 'error');
             // Do NOT throw here, so the dashboard still loads (even if empty)
          }

          let goalData: Goal | null = null;
          try {
             goalData = await api.getStudentGoal(userProfile.name);
          } catch (e) {
             console.warn("Could not fetch goals (likely permission issue, ignoring):", e);
          }

          let studentLessonPlan: LessonPlan | null = null;
          try {
             if (userProfile.grade) {
                studentLessonPlan = await api.getLessonPlanForGrade(userProfile.grade);
             }
          } catch (e) {
             console.warn("Could not fetch lesson plan (likely permission issue, ignoring):", e);
          }
          ...
          // We rely on updated firestore rules to allow this.
          try {
              ...
              // Fallback to just own entries if global fetch fails (e.g. strict rules)
...
${fingeringKnowledge}`,
  ...
  const handleEnterGuestMode = () => {
    const userEntries = generateMockEntries(mockUserProfile);
    const communityEntries = generateCommunityMockEntries();
    
    setUserProfile(mockUserProfile);
    setPracticeEntries(userEntries);
    setAllPracticeEntries([...userEntries, ...communityEntries].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
    setGoals(mockGoals);
    setIsGuestMode(true);
    setAuthLoading(false);
    setIsLoading(false);
